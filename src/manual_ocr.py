...
class ReceiptOCR:
    """Preprocess images with via `pil` and `opencv` and extract text via ocr techniques"""
    ...
    def ocr_with_easyocr(self, processed_images: List):
    ...
    def extract_text_from_easyocr_results(self, processed_images:List):
        """Perform OCR and extract the text data via easyocr"""
        easy_extractions = self.ocr_with_easyocr(processed_images)

        all_text = []
        for extraction in easy_extractions:
            text = [ex[1] for ex in extraction]
            text = " ".join(text)
