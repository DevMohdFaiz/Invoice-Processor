...
class ReceiptDB():
    ...
    def __init__(
            self,
            sql_db_path="db/receipts_sql_db",
            sql_db_name="receipts.db",
            vector_db_path="db/receipts_vector_db",
            vector_db_name="receipts",
        ...
        try:
            ...
            raise FileNotFoundError(f"{full_receipt_details} not found!")
        self.sql_db_path = sql_db_path
        self.sql_db = os.path.join(sql_db_path, sql_db_name)
    ...
    def create_sql_db(self, db_path=None):
        ...
        Args:
            db_path:str = path of the sql database
        """
        if db_path is None:
            db_path= self.sql_db_path

        if not os.path.exists(db_path): #create the sql db dir if it doesn't exist
            os.makedirs(db_path)
        ...
        with sqlite3.connect(self.sql_db) as conn:
            ...
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_receipt_id ON line_items(receipt_id)")
    ...
    def store_receipt(self, sql_db=None):
        ...
        with sqlite3.connect(self.sql_db) as conn:
            ...
            for receipt in tqdm(self.full_receipt_details, desc="Creating receipts db..."):
                additional_info = receipt.get('additional_information')
    ...
    def create_vector_db(self,  receipt, receipt_id, vector_db_path=None,):
        ...
        # print(f"{self.vector_db_name} successfully created at {self.vector_db_path}")
