...
def optimize_images_for_lvm(orig_images_path, optimized_images_path, max_img_size=1024):
    ...
    for img_path in tqdm(orig_images, desc="Optimizing images for LVM..."):
        img= Image.open(orig_images_path/img_path).convert("RGB")
        opt_img_path = os.path.join(optimized_images_path, img_path)

        if max(img.size) > max_img_size:
            ratio = max_img_size / max(img.size)
            new_size = (int(img.width * ratio), int(img.height * ratio))
            opt_img = img.resize(new_size, Image.LANCZOS)
            opt_img.save(opt_img_path, optimize=True, quality=85)
        else:
            img.save(opt_img_path, optimize=True, quality=85)

def encode_image_tobase64(image_path):
    """Convert user Image to base64 string"""
    with open(image_path, "rb") as img_file:
        return base64.b64encode(img_file.read()).decode("utf-8")

def extract_receipt_data(image_path):
    base64_image = encode_image_tobase64(image_path)
    llm = ChatGroq(api_key=GROQ_API_KEY, model="meta-llama/llama-4-scout-17b-16e-instruct", temperature=0)
